...
exports.create = async function(req, res) {
    var authorization = await Authorization.authorize({ user_id: req.body.user_id })
    if (authorization) {
        if (req.body.unique_name) {

            // Check Unique name should be at least 6 characters length

            if (req.body.unique_name.length < 6) {
                res.status(201).json({
                    status: "Fail",
            ...
            var count_unique_name = await business.find({ created_by: req.body.user_id, unique_name: req.body.unique_name }).count();
            ...
            var unique_code = await generateCode.getNextCode(req.body.unique_name);

            // Store business in database

            const newbusiness = new business({
                bid: await ExistsCodeGenerate(),
                created_at: new Date(),
...
exports.edit = async function(req, res) {
    ...
    if (authorization) {
        var query = { created_by: req.body.user_id, bid: req.body.bid };
        var valueUpdate = { $set: { name: req.body.name, decription: req.body.decription } };
